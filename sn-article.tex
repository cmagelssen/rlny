...
At a broad level, our findings that reinforcement learning improved more during the acquisition sessions and showed better retention than supervised (free choice) learning corroborated earlier studies showing that reinforcement learning enhances skill retention \cite{therrien_effective_2016, truong_error-based_2023, hasson_reinforcement_2015, lior_shmuelof_overcoming_2012}. An explanation for these findings is that reinforcement learning stimulates the slow-learning processes that promote long-term learning \cite{huang_rethinking_2011, uehara_learning_2018}. The better race times in reinforcement learning also align with prior studies on the 'discovery learning' approach, where practice without explicit instruction yields better learning effects \cite{wulf_instructions_1997, hodges_learning_2001, hodges_role_1999}. Together, these findings suggest that alternative teaching strategies can be useful for training skills.

Our proposed explanation for the enhanced learning in the reinforcement learning group, however, was that they learned to make better strategic decisions. Given that we did not find compelling evidence that reinforcement learning outperformed the supervised (target skill) learning group, which was coached in our theoretically optimal strategy, but did so relative to the group using their own coaches (supervised (free choice) learning), this explanation may hold merit. However, we found that this was not the case when we examined group differences in the probability of choosing the best strategy, neither for the theoretically optimal strategy nor for the individual skier's estimated best strategy for each skier. On the other hand, we found that the reinforcement learning group had a lower 'cost'—the expected difference between the chosen 'suboptimal' strategy and the skier's estimated best strategy—during retention. One reason we only discovered better choices by studying costs is that skiers may have found it difficult to distinguish between certain strategies due to their marginally small time differences.
...
%It should also be noted that the coaches in the supervised (free choice) learning group were highly experienced and had access to the skiers' trial race times as well. Based on the evaluations, it appears that the coaches also had a significant learning effect, at least descriptively, which 
...
En begrensning med studien er at vi ikke hadde motion capture for å studere utøvernes utførelse av strategiene. Vi kan dermed ikke uttale oss om faktisk utførelse av strategiene, annet enn at vi verifiserte at de klarte å utføre strategiene under tilvenningstesten. Imidlertid fulgte vi imidlertid anbefalingen om å lage 
