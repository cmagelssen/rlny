...
If reinforcement learning were to learn to select better strategies, we would expect to observe differences in improvement among the three learning groups over the course of the three acquisition sessions. Therefore, our hypothesis was that the reinforcement learning group would improve more during the acquisition sessions than the two supervised learning groups. There were no baseline differences between the three groups: During the first acquisition session (Forced Exploration), we found that the average race times did not significantly differ between the reinforcement learning group and the supervised (free choice) learning group ($\beta$ = 0.06 , 95\% CI [-0.2, 0.32], $t$(92.727) = 0.48, $p$ = 0.631),  and the supervised (target skill) learning group ($\beta$ = 0.18, 95\% CI[-0.08, 0.44], $t$(92.663) = 1.37, $p$ = 0.174). By contrast, we would expect differences in improvement over the course of the second (Free Choice 1) and third (Free Choice 2) acquisition session, when the skiers or the coach had the autonomy to pick strategy. We found that all treatment groups significantly improved their race times over the course of the Free Choice sessions during acquisition (Supplementary Table 1), suggesting that the choice of strategy influenced the race time. We found, however, that the rate of improvement was different for three groups. 


%improvement was significantly greater for supervised (target skill) learning, with coaches selecting the theoretical best strategy, compared with reinforcement learning ($\beta$ = -0.12, 95\% CI[-0.22, -0.03], $t$(91.777) = -2.58, $p$ = 0.012).




Over the course of the three acquisition sessions, we would expect to observe differences in improvement among t

our explanation that reinforcement learning learns to improve strategies by objectively evaluating them through observing race times is correct, we would expect to see differences in improvement among the different groups over the course of three acquisition sessions. We would not, however, expect group differences during Forced Exploration, however, 

 
Over the course of the three acquisition sessions,

Over the course of the three acquisition sessions, we would expect to observe differences in improvement among the groups if reinforcement learning were to learn to select better strategies. Therefore, our hypothesis was that reinforcement learning would improve more during acquisition than supervised learning. During Forced Exploration, we found that the average race times did not significantly differ between reinforcement learning and supervised (free choice) learning ($\beta$ = 0.06 , 95\% CI [-0.2, 0.32], $t$(92.727) = 0.48, $p$ = 0.631) and supervised (target skill) learning ($\beta$ = 0.18, 95\% CI[-0.08, 0.44], $t$(92.663) = 1.37, $p$ = 0.174). This lack of statistically significant differences between the groups was expected, however, since all groups had the same number of trials on each strategy in this session. In contrast, group differences in race time improvement were expected during the next two acquisition sessions (Free Choice 1 and Free Choice 2), when the skiers or the coach had the autonomy to pick strategy. We found that all treatment groups significantly improved their race times from Forced Exploration to Free Choice 1 (Supplementary Table 1). However, this improvement was significantly greater for supervised (target skill) learning, with coaches selecting the theoretical best strategy, compared with reinforcement learning ($\beta$ = -0.12, 95\% CI[-0.22, -0.03], $t$(91.777) = -2.58, $p$ = 0.012). In contrast, supervised (free choice) learning, where coaches made strategy choices for the skiers, improved descriptively less from Forced Exploration to Free Choice 1 than reinforcement learning, but the interaction effect was not statistically significant ($\beta$ = 0.08, 95\% CI[-0.02, 0.17], $t$(92.5) = 1.61, $p$ = 0.110).
...
In the first acquisition session (Forced Exploration), we did not expect significant differences between the groups, however, since all groups had the same number of trials on each strategy. 
...
Vi antok først at den teoretisk beste strategien var den beste strategien og testet om reinforcement learning hadde hadde høyere sannsynlighet enn supervised (free choice) learning for å velge denne (note that it was not possible to include supervised (target skill) learning in this model because they always chose this strategy). The figure \ref{fig: choice_estimated}a displays the predicted probabilities for the treatment groups across the sessions. Vi fant at begge gruppene signifikant økte valget av den teoretisk beste strategien over treningsøktene relativt til Free Choice 1, men økningen oppstod på forskjellige tidspunkt for de to gruppene (see tabell). Under Free Choice 1 og Free Choice 2 var begge gruppene konservative til å velge denne strategien. Da skikjørerne i supervised learning (free choice) ble løsluppet fra treneren og kunne velge strategier selv på retention fant vi en signifikant øktning fra Free Choice 1. For reinforcement learning fant vi derimot ingen signifikant økning frem til retention. Forskjellen i endring mellom gruppene var statistisk signifkant. Da utøverne ble testet i transfer løypen fant vi at reinforcement learning signifikant økte deres valg av den teoretisk beste strategien relativt til Free Choice 1. Vi fant riktignok ingen statistisk signifikante forskjeller mellom reinforcement learning og supervised (free choice) learning på noen av øktene. 
...
For reinforcement learning inntraff økningen da  


their relatively to Free Choice 1 (see table). 



groups were conservative in picking this strategy during Free Choice 1 and Free Choice 2. 




groups statistically significantly increased their predicted probability of choosing the theoretical best strategy over the sessions, but at different times. The reinforcement learning group remained conservative in choosing. However, we did not find any significant differences between the groups at any of the sessions, nor their interaction   
