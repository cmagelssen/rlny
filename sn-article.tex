...
Making brilliant strategy choices to improve performance is a distinctive characteristic of expertise. Typically, these strategies come from a coach who provides knowledge on which strategies to adopt. Here, we instead asked whether shifting from an instruction (supervised learning) to a more evaluation-oriented (reinforcement learning) training strategy can improve strategy decisions and consequently enhance performance in skilled performers. In a three-day skill learning experiment involving ninety-eight skilled alpine ski racers, we found that the reinforcement learning group, which learned to select effective strategies through evaluations based on trial feedback, achieved greater improvement over the course of the acquisition sessions and outperformed the supervised (free choice) learning group during retention. In the supervised learning group, skiers were paired with a coach who selected strategies for them. As expected, we did not find evidence that the reinforcement learning group learned better than the supervised (target skill) learning group, which served as a benchmark group for the upper limit of performance achievable through optimal strategy choices. No convincing evidence corroborated our hypothesis that reinforcement learning improved transfer. 

Purely based on performance, our findings that reinforcement learning improved more during the acquisition sessions and showed better retention than the supervised (free choice) learning group corroborates earlier studies that showed that reinforcement learning enhances skill retention \cite{therrien_effective_2016, truong_error-based_2023, hasson_reinforcement_2015, lior_shmuelof_overcoming_2012}. An explanation for these findings is that reinforcement learning stimulates the slow-learning processes that promote long-term learning \cite{huang_rethinking_2011, uehara_learning_2018}. The better race times in reinforcement learning also align with prior studies on the 'discovery learning' approach, where practice without explicit instruction yields better learning effects \cite{wulf_instructions_1997, hodges_learning_2001, hodges_role_1999}. As we did not find convincing evidence that reinforcement learning improved learning compared to the supervised (target skill) learning group, antyder dette har en annen forklaring.

Our proposed explanation was that the better learning in the supervised (free choice) learning group was due to better strategy selection. We did not, however, find convincing evidence that this was the case for the best choice; neither for the theoretical best strategy nor for the individual skier's estimated best strategy. On the other hand, we found that the reinforcement learning group had a lower 'cost' (that is, expected difference between the chosen 'suboptimal' strategy and the skier's estimated best strategy) during retention, suggesting that they had better learned to choose strategies that did not compromise performance. En forklaring på dette er at det kunne være vanskelig for utøverne å skille noen av strategiene på grunn av marginale tidsforskjeller, som gjorde at vekslet mellom disse. Det skal også legges til at trenerne også var meget erfarne trenere og at også de hadde tilgang til utøvernes tider. Ut fra evalueringene kan det synes som at trenerne også had en betydelig læringseffekt, i det minste deskriptivt. Endringene i evaluering var ikke like store som hos reinforcement learning, men en del av forklaringen på at disse ikke nådde signifikans kan tenkes skyldes færre observasjoner i supervised (free choice) learning gruppen siden det var trenerne som rangerte strategiene i begynnelsen og ikke utøverne. 
...
Surprisingly, despite the broad consensus that "extend with rock skis forward" was the best strategy, at least on average, coaches and skiers faced greater challenges in determining which of the two strategies, 'rock skis forward' and 'extend', was the primary mechanism behind its effect. Over the course of the next sessions, we found that the reinforcement learning group learned to distinguish these strategies from each other, as evidenced by a significantly improved assessment of 'extend' and a poorer assessment of 'rock skis forward', which we did not find in the other two groups. In addition, we found that the reinforcement learning group ranked “rock skis forward” more poorly over the course of the sessions than did the supervised (target skill) learning group. This surprising and interesting finding suggests that learners may miss potential learning by only being exposed to one strategy instead of a broader exploration of alternatives, as was the case with the supervised (target skill) learning group. Det å bli exponert for reinforcement learning type trening kan se ut til å ha stimulert resonneringsprosessen hos utøveren slik at de bedre forstod hvordan deres handlingene påvirket utfallet 


til å evaluere strategiene gjennom reinforcement learning kan se ut til å ha gjort utøvere til å flinkere til å resonnere 



Over time, such exploration could prove crucial in developing innovative strategies, as athletes cultivate a deeper comprehension of the relationship between their actions and performance outcomes \cite{ericsson_scientific_1998}.




may miss potential learning by only being exposed to one strategy instead of a broader exploration of alternatives. 





that the reinforcement learning assessed 'rock skis forward'     



there was great overlap in what the skiers and coaches believed to be the most important underlying mechanism of 'rock skis forward' and 'extend' despite clear differences in their race time performance. Across the sessions, we found that the reinforcement learning group better learned to dissociate these strategies from one another, evidently by the significant slope for this but not for the other two groups.   
